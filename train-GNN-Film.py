...
# from torch_geometric.nn import R
...
#         self.convs = torch.nn.ModuleList()
#         self.relu = F.relu
#         self.convs.append(HGTConv(in_channels, hidden_channels,  heads=n_heads,metadata=None))
#         for i in range(n_layers - 2):
#             self.convs.append(HGTConv(hidden_channels, hidden_channels,
#                                        heads=n_heads,metadata=metada))
#         self.convs.append(HGTConv(hidden_channels, hidden_channels,
#                                    heads=n_heads,metadata=None))
#         self.lin1 = torch.nn.Linear(hidden_channels)
#
...
class GraphSAGE_Net(torch.nn.Module):
    def __init__(self, festures, hidden, classes):
        super(GraphSAGE_Net,self).__init__()
        self.convs = torch.nn.ModuleList()
        self.gat1 = SAGEConv(festures,hidden)
        self.gat2 = SAGEConv(hidden,classes)

    def forward(self, x, edge_index=2651):
        for i, conv in enumerate(self.convs):
...
#         self.dropout = dropout
#         self.convs = torch.nn.ModuleList()
#         self.convs.append(FiLMConv(in_channels,hideden_channels,num_relations=24))
#         for _ in range(n_layers-1):
#             self.convs.append(FiLMConv(hideden_channels,hideden_channels,num_relations=2))
#         self.norms = torch.nn.ModuleList()
#         for _ in range(n_layers):
#             self.norms.append(torch.nn.BatchNorm1d(hideden_channels))
#         self.lin_l = torch.nn.Sequential(collections.OrderedDict([
#             ('lin1',torch.nn.Linear(hideden_channels,int(hideden_channels//4),bias=False)),
#             ('lrelu',torch.nn.LeakyReLU(0.2)),
#             ('lin2',torch.nn.Linear(int(hideden_channels//4),out_channels,bias=False))
#         ]))
#     def forward(self, x, edge_index=2561, edge_type=24):
#         for conv, norm in zip(self.convs, self.norms):
#             x = norm(conv(x, 2561, 24)).view()
#             print(x.shape)
#             x = F.relu(x)
#             x = F.dropout(x, p=self.dropout, training=self.training)
#         x = self.lin_l(x)
...
class REModel(nn.Module):
    ...
    def forward(self, token_ids, seg_ids, sub_ids=None):
        ...
        # out1 = GraphSAGE_Net(festures=2, hidden=16, classes=24).to(DEVICE)(out1)
        # print(1)
        # print(out1.shape)
        # print(1)
        # print(out1.shape)
...
def train(model, train_loader, optimizer, epoches, device):
    ...
    for _ in range(epoches):
        ...
        for batch_idx, x in tqdm(enumerate(train_loader)):
            ...
            # loss_obj = Dice_loss(sub_preds, sub_labels)
            loss = sub_dice_loss + obj_dice_loss
            optimizer.zero_grad()
